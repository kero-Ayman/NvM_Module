...
/* Provide fallback module id when Modules.h is not available in test workspace */
#ifndef MODULE_ID_NVM
#define MODULE_ID_NVM 0xA0
#endif

/* Map lowercase boolean constants to AUTOSAR TRUE/FALSE when stdbool.h isn't used */
#ifndef true
#define true TRUE
#define false FALSE
#endif

/* Provide `bool` alias to AUTOSAR boolean */
typedef boolean bool;

/* Fallbacks for IRQ/OS abstractions used by the original project */
typedef uint32 imask_t;
static inline void Irq_Save(imask_t x) { (void)x; }
static inline void Irq_Restore(imask_t x) { (void)x; }

/* Fallback configuration macros used by NvM when full config headers are not present */
#ifndef NVM_MAX_NUMBER_OF_WRITE_RETRIES
#define NVM_MAX_NUMBER_OF_WRITE_RETRIES 3
#endif

/* Job queue and buffer size defaults for test builds */
#ifndef NVM_SIZE_STANDARD_JOB_QUEUE
#define NVM_SIZE_STANDARD_JOB_QUEUE 8
#endif

#ifndef NVM_MAX_BLOCK_LENGTH
#define NVM_MAX_BLOCK_LENGTH 512
#endif

/* Simple read/write helpers for unaligned access in test environment */
#ifndef READ16_NA
#define READ16_NA(ptr) (*(uint16_t*)(ptr))
#endif
#ifndef READ32_NA
#define READ32_NA(ptr) (*(uint32_t*)(ptr))
#endif
#ifndef WRITE16_NA
#define WRITE16_NA(ptr,val) (*(uint16_t*)(ptr) = (uint16_t)(val))
#endif
#ifndef WRITE32_NA
#define WRITE32_NA(ptr,val) (*(uint32_t*)(ptr) = (uint32_t)(val))
#endif
...
static boolean CheckMemIfJobFinished(void)
{
	...
	if (!MemIfJobAdmin.JobFinished) {
		/* Use device 0 for MemIf job result when no descriptor is available */
		uint8 devId = 0;
		jobResult = MemIf_GetJobResult(devId);
 ...
 * Reentrant:	No
 */
/* NvM_JobEndNotification is provided by application callback stub (NVM_cbk.c).
   Keep NvM internal definition disabled to avoid duplicate symbols in test build. */

#if 0
void NvM_JobEndNotification(void)
{
	/* Device id is fixed to 0 in this test/stub environment */
	uint8 devId = 0;
	MemIfJobAdmin.JobFinished = TRUE;
	MemIfJobAdmin.JobStatus = E_OK;
	MemIfJobAdmin.JobResult = MemIf_GetJobResult(devId);
}
#endif
 ...
 * Reentrant:	No
 */
/* NvM_JobErrorNotification is provided by application callback stub (NVM_cbk.c).
   Disabled here to avoid duplicate symbols in test build. */
